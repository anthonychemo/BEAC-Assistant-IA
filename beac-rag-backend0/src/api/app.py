...
from src.rag.llm_client import get_default_llm, get_llm, OPENROUTER_MODELS
...
app = FastAPI(title="BEAC RAG API", version="1.0.0", lifespan=lifespan)
...
def health() -> HealthResponse:
    ...
    return HealthResponse(
        status="ok", documents=n_docs, chunks=n_chunks, statistics=n_stats,
        llm_model=list(OPENROUTER_MODELS.values())[0],
...
def query(req: QueryRequest) -> QueryResponse:
    try:
        result = answer_question(req.question, model_key=req.model_key)
    except Exception as exc:
...
def get_documents(
    ...
    for doc in docs:
        ...
        result.append({
            ...
            "source_path": doc.source_path, "country": doc.country,
        })
